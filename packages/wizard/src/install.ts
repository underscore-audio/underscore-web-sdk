 ...
 *   1. Install `@underscore-audio/sdk`, `supersonic-scsynth`, and
 *      `supersonic-scsynth-core` using the project's package manager
 *      (as detected in detect.ts). Peer ranges come from the SDK
 *      package.json via sdk-peers.ts.
...
function installCommand(
  ...
  /*
   * sdkInstallPackages() entries may include version specifiers (e.g.
   * "supersonic-scsynth@>=0.70.0 <1.0.0") so we strip the @version
   * tail when looking up the overrides map. Override keys are bare
   * package names ("supersonic-scsynth"), which matches how a caller
   * would write them.
   */
  const resolved = sdkInstallPackages().map((spec) => {
...
export async function installDependencies(
  ...
  const packages = sdkInstallPackages();
  const { cmd, args } = installCommand(project.packageManager, options.tarballOverrides);

  const spinner = p.spinner();
  spinner.start(`Installing ${packages.join(" + ")} with ${pc.cyan(project.packageManager)}...`);
  try {
...
export async function copyWasmAssets(
  ...
  try {
    await run("npx", ["underscore-sdk", target], { cwd: project.root });
