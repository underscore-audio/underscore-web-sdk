 ...
 *    existing file and rerun `npx @underscore/wizard`.
...
export function wrapperSource(framework: Framework): string {
  if (framework === "vanilla-html") {
    return `/*
 * Thin wrapper around @underscore/sdk. Imported by underscore-demo.html.
 *
...
`;
  ...
  return `/*
 * Thin wrapper around @underscore/sdk.
 *
...
if (!apiKey) {
  throw new Error(
    "${keyName} is not set. Re-run \`npx @underscore/wizard@latest\` or set it manually."
  );
}
...
/*
 * Demo component generated by \`npx @underscore/wizard\`. Click "Play" to
...
/*
 * Demo wired by \`npx @underscore/wizard\`. Call bindPlayButton with any
...
<html>
  ...
  <body>
    ...
    <script type="module">
      import { Underscore } from "/node_modules/@underscore/sdk/dist/index.js";
