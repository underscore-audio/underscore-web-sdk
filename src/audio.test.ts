...
describe("AudioEngine", () => {
  ...
  describe("masterVolume", () => {
    ...
    it("is independent of per-synth setParam('amp', x): amp cache is unaffected", () => {
       ...
       * UI slider to ride above a score's amp automation. Test the
       * observable: setMasterVolume must not write into the
       * engine.paramValues cache, and amp set via setParam (no-op
       * pre-init, but still tracked) must not be affected by master
       * volume motion.
       */
