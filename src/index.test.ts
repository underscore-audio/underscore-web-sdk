...
describe("Underscore", () => {
  ...
  describe("loadSynth()", () => {
    ...
    /*
     * Two concurrent loadSynth calls for the same composition/synth must
     * coalesce onto a single in-flight promise. Without dedup, the second
     * caller races the synthdef blob URL revocation done inside
     * loadSynthdefFromData and ends up stuck waiting on a fetch that the
     * first call has already revoked.
     */
    it("dedupes concurrent loadSynth calls for the same (compositionId, synthName)", async () => {
      const metadata = {
        name: "warm_pad",
        description: "A warm pad",
        params: [],
        createdAt: "2024-01-01",
        synthdefUrl: "/api/v1/compositions/cmp_x/synths/warm_pad/synthdef",
      };
      ...
      const [s1, s2] = await Promise.all([
        client.loadSynth("cmp_x", "warm_pad"),
        client.loadSynth("cmp_x", "warm_pad"),
      ]);

      expect(s1).toBe(s2);
      expect(s1).toBeInstanceOf(Synth);
      /*
       * One getSynth + one fetchSynthdef. A second concurrent call must
       * not trigger additional HTTP work; if it did, mockFetch would
       * resolve with `undefined` and the assertion below would catch the
       * regression.
       */
      expect(mockFetch).toHaveBeenCalledTimes(2);
    });

    it("releases the inflight entry so a later loadSynth can run again", async () => {
      const metadata = {
        name: "warm_pad",
        description: "A warm pad",
        params: [],
      ...
      queueOk();
      const client = new Underscore({ apiKey: "us_test_key" });
      await client.loadSynth("cmp_x", "warm_pad");

      queueOk();
      await expect(client.loadSynth("cmp_x", "warm_pad")).resolves.toBeInstanceOf(Synth);
